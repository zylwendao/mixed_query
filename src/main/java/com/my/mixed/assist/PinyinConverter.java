...
//		chinese = chinese.replaceAll("[\\.，\\,！·\\!？\\?；\\;\\(\\)\\[\\]\\:： ]+", " ").trim();
	...
	private String convertFirstToUpperCase(String str) {
		if (str == null || str.length()==0) {
			return "";
		}
	...
	public String getPinyinAbbreviation(String chinese) throws IllegalPinyinException {
		if(StringUtils.isEmpty(chinese)){
			return null;
		}
...
//		chinese = chinese.replaceAll("[\\.，\\,！·\\!？\\?；\\;\\(\\)（）\\[\\]\\:： ]+", " ").trim();
		...
		for(int i=0;i<chs.length;i++){
			String[] py_arr = getPinyin(chs[i]);
			if(py_arr==null || py_arr.length<1){//特殊字符
				py_sb.append(chs[i]);
				continue;
//				throw new IllegalPinyinException("pinyin array is empty, char:"+chs[i]+",chinese:"+chinese);
			}
			if(py_arr.length==1){
				py_sb.append(convertFirstToUpperCase(py_arr[0]));
			}else if(py_arr.length==2 && py_arr[0].equals(py_arr[1])){
				py_sb.append(convertFirstToUpperCase(py_arr[0]));
			}else{
				String resultPy = null, defaultPy = null;;
				...
				py_sb.append(convertFirstToUpperCase(resultPy));
			}
		}
